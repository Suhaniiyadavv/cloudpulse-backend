...
# ============================================================
# Compatibility patch for sklearn SimpleImputer on some deploy envs
# Fix: AttributeError: 'SimpleImputer' object has no attribute '_fill_dtype'
# ============================================================
try:
    import sklearn
...
# Some pickled SimpleImputer objects may miss internal attrs on different envs
if ae_imputer is not None:
    if not hasattr(ae_imputer, "_fill_dtype"):
        ae_imputer._fill_dtype = np.float64
        print("[PATCH] Added missing ae_imputer._fill_dtype")

    if not hasattr(ae_imputer, "_fit_dtype"):
        ae_imputer._fit_dtype = np.float64
        print("[PATCH] Added missing ae_imputer._fit_dtype")
